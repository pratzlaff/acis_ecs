...
import sys
if len(sys.argv) != 5: sys.exit('\nUsage: python *.py epoch fp_temp[see script opts] yes/no[TG] [bin]\n\n')
(sname,e,fptv,tg,bin)= sys.argv
bin= int(bin)
...
if test==1:
    ...
    xstart=1
    ystart=1
    testerr=1   ## 0= turn conf errors off for test run
...
pdir= pdir_tmp.name
os.environ['PFILES']='{};/soft/ciao/contrib/param:/soft/ciao/param:'.format(pdir)


if e=='e16a' or e=='e16b' or e=='e16c':
    enum= 16
else:
    enum= int(e.split('e')[1])
fptv=int(fptv)
...
if bin==3232:
    sbin='32x32y'; xbin=32; ybin=32; bscl= 1/64
if bin==3264:
    sbin='32x64y'; xbin=32; ybin=64; bscl= 1/32
if bin==32256:
    sbin='32x256y'; xbin=32; ybin=256; bscl= 1/8
if bin==32128:
    sbin='32x128y'; xbin=32; ybin=128; bscl= 1/16
if bin==64256:
    sbin='64x256y'; xbin=64; ybin=256; bscl= 1/4
if bin==128256:
    sbin='128x256y'; xbin=128; ybin=256; bscl= 1/2
if bin==256256:
    sbin='256x256y'; xbin=256; ybin=256; bscl= 1
...
spec_dir= '{}/spec/{}/fpt_{}_{}_{}TG{}/'.format(basedir,e,sfpt,sbin,tg,xtra)
fit_dir= '{}/fits/{}/fpt_{}_{}_{}TG{}/'.format(basedir,e,sfpt,sbin,tg,xtra)
plt_dir= '{}/figs/'.format(fit_dir)
...
dgdir= '/soft/ciao/CALDB/data/chandra/acis/det_gain/'
    ...
    for xl in range(xstart,1025,xbin):
        ...
        for yl in range(ystart,1025,ybin):
            ...
            if os.path.exists(pif):
                ...
                rmf_dir= '/data/hal9000/acis_response/rmf/pi_{}x{}y_{}'.format(xbin,ybin,rmf_fpt)
            ...
            if tot_cnts>cnt_thresh:
                ...
                bgf,= glob('/data/hal9000/acis_bkg/s04_final_models_nolines/{}_{}_{}-*y.dat'.format(bccd,byear,byl))
            ...
            if recpars==1:
                ...
                if opentxt==0:
                    ...
                    fh_ecs.write('#### epoch {}\tstart= {:.3f}\tksec= {:.1f}\n'.format(e,dyear,expo))
                    fh_bkg.write('#### epoch {}\tstart= {:.3f}\tksec= {:.1f}\n'.format(e,dyear,expo))
                    fh_pha.write('#### epoch {}\tstart= {:.3f}\tksec= {:.1f}\n'.format(e,dyear,expo))                    
